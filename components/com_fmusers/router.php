...
/**
 * Function to build a Fmusers URL route.
 *
 ...
 * @since	1.5
 */
function FmusersBuildRoute(&$query)
{
	...
	if (empty($items)) {
		...
		$menu	= $app->getMenu();
		$items	= $menu->getItems('component', 'com_fmusers');
		...
		// Set the default menu item to use for com_fmusers if possible.
		if ($profile) {
			$default = $profile;
		} elseif ($registration) {
