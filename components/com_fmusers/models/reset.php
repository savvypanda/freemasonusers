...
/**
 * Rest model class for Fmusers.
 *
 * @package		Joomla.Site
 * @subpackage	com_fmusers
 * @since		1.5
 */
class FmusersModelReset extends JModelForm
{
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_fmusers.reset_request', 'reset_request', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_fmusers.reset_complete', 'reset_complete', $options = array('control' => 'jform'));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_fmusers.reset_confirm', 'reset_confirm', $options = array('control' => 'jform'));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		/* Commenting out as the reset confirmation should be
		 * done entirely by the token in the email link
		 ...
		 * End commented out section. Now we need to confirm the token from the email
		 */

		// Find the user id for the given token.
		$activation = JRequest::getString('token');
		if(empty($activation)) {
			$this->setError(JText::sprintf('COM_USERS_RESET_COMPLETE_TOKENS_MISSING'));
			return false;
		}
		$db	= $this->getDbo();
...
/* commented out since the activation code is all that is checked
...
*/
		...
		$app = JFactory::getApplication();
		$app->setUserState('com_users.reset.token', $activation); //$crypt.':'.$salt);
	...
	public function processResetRequest($data)
	{
		...
		$link = 'index.php?option=com_fmusers&task=reset.confirm&token='.$token;
